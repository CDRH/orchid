...
- facet selection and search button added to browse pages for long lists of 
  facet results
- ability to facet by a nested field, where another nested field equals a given
 value`person.sex[person.role#person]` will facet on the `sex` subfield of the 
 `person` field, but only for items where the `role` subfield equals "person"
