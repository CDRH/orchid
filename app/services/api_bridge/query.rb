...
module ApiBridge
  ...
  class Query
    ...
    def should_display?(facet)
      if facet.class == String
        # normal facet, account for things like "date.year"
        info = page_facets[facet]
      elsif facet.class == Array
        # nested/bucket aggregations
        info = page_facets[facet[0]]
      end
      facet.present? && info && info["flags"] \
